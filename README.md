...
- 🔍 **Final Quality Pass / Review**: Seksjonsvis post-check er pensjonert. Review-first quality gate, targeted/broad revision og QA Memo er samlet rundt `ai-final-review`, `hooks/useAutoQualityGate.ts`, `services/finalReview/*` og review-progresjon på tvers av revision branches. Q2-grunnleveransene for `Multi-Round Final Review` og `Guided Patch Preview` er nå på plass med QA-historikk, review-delta, stagnasjons-/regresjonssignaler, patch-kandidater, scope-planlegging og QA-prefilled revise-briefs; videre arbeid er primært operator-UX, sortering og oppfølging av review-helse.
...
- 🧠 **Suggest Settings Rebalancing**: Prompt-, heuristikk- og normaliseringsgrunnlaget er samlet i delt lag med fast-path, fallback og strengere genre-/opsjonsnormalisering. Dette lukker Q2-rebalancing som hovedleveranse; videre arbeid er bias-/routing-tuning på tvers av category/genre-kombinasjonene.
