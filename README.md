...
- 🌐 **Oversettelses- og variantgrunnlag**: `ai-translate-plan` og `ai-translate-markdown` etablerte server-side språkvariantflyt, mens `Variant` fikk egen save-target-logikk og samme låste profil-kontrakt som importert Story Engine-fil.
- 🧭 **Projects UX og statusvisning**: Prosjektkort og prosjektåpning ble komprimert og tydeliggjort med fargekodet status, bedre neste-steg-guidance og admin-skjult advanced mode.
- 👥 **Admin Users Projects-tab**: Admin-brukerflaten fikk prosjektvolum, review health, revision branches og siste prosjektaktivitet via paginert `ai-admin-user-projects`.
- 🎙️ **Gemini 3.1 Flash TTS**: Ny TTS-modell ble lagt til, lang audio-segmentering ble mer robust, og legacy daily TTS quota ble erstattet av kredittbasert billingtekst.
- 🖼️ **GPT Image 2 og visuelle referanser**: `gpt-image-2` og `gpt-image-2-hd` ble lagt til som hovedvalg for bildegenerering, med bedre retries og mer robust Core Idea-flyt for opplastede visuelle referanser.
- 💳 **Usage ledger og kredittreservasjoner**: Usage ledger og image credit reservations ble lagt til, som grunnlag for mer presis produksjonsrapportering og idempotent kredittføring.
- 🧠 **Final Review-modell og hybridprising**: Final Review/Revision ble flyttet til GPT-5.5 medium som review-standard, med hybrid pricing defaults og provider-prising i Production Report.
- 🧠 **Suggest Settings-rebalansering**: Prompt-, heuristikk- og normaliseringsgrunnlaget ble samlet i delt lag med fast-path, fallback og strengere genre-/opsjonsnormalisering.

### Siste endringer Mai 2026
- 🧭 **Simple / Custom startmodus**: Startsiden fikk en enklere `Simple`-modus for førstegenerering, der avanserte kontroller skjules og `Suggest Settings` kjøres automatisk ved `Create Project`. `Custom` beholder full kontrollflate.
- ✅ **Final Quality Pass / Review**: Seksjonsvis post-check ble pensjonert til fordel for review-first quality gate, targeted/broad revision, QA Memo, QA-historikk, review-delta og QA-prefilled revise-briefs.
- 🔒 **Strukturert fact-lock**: `ai-plan` normaliserer Core Idea-URL-er, opplastede dokumenter, Google Search-kilder og produkt-/konfiguratorlenker til én kildepool, rangerer kilder og løser konflikter før primary facts sendes videre.
- 🔎 **Verified Research Sources v2**: `ai-plan` fikk rik `verifiedCitations`-metadata med source tiers, safe-fetch/SSRF-beskyttelse, bounded reads, soft404/mismatch-signaler og fallback til enkel citation-kontrakt.
- 🧾 **Fact candidates og debug**: Story Engine-filen kan eksportere kildepool, faktakandidater, primary facts, rejected/conflict facts og source-backed status i frontmatter når fact-lock er aktiv.
...
### Siste endringer Juni 2026
- 💬 **Startskjerm-hjelpetekster**: `Upload Files`, `Final Quality Pass` og kreditt-preflight fikk kortere, mer lesbare hjelpetekster i Simple og Custom, med tydeligere kostnads-/kvalitetsforklaring og safety-forbehold for opplastet media.
- 🛡️ **Opplastet media-sikkerhet**: `ai-analyze-file` legger inn en generell high-stakes media guard for bilde/lyd/video slik at analyser beskriver observerbare trekk, markerer usikkerhet og anbefaler kvalifisert verifisering i risikobærende temaer.
- 🛡️ **Evidence safety i preview og eksport**: Utkast med ulukket kildestatus kan vise samme lokaliserte kildestatus-varsel i workspace preview og eksporterte formater. Export warning-tekstene er flyttet til delt i18n-lag.
...
- 🧭 **Publiseringsstatus og source/UI-kalibrering**: Prosjektbadges, stale warning-håndtering, source display titles, URL-labels og high-assurance source repair ble strammet inn uten å hardkode konkrete domener eller emner.

### Siste endringer Juli 2026
...
- 🖼️ **Lokaliserte språkvariantbilder**: Cover og seksjonsbilder for språkvarianter regenereres med valgt skriftspråk der språkbytte faktisk krever nye visuelle leveranser.
- 🎧 **Medie-only Variant**: Audio-varianter kan oppdatere samme prosjekt uten å regenerere eller miste eksisterende seksjonsbilder når `Illustrations` ikke er valgt. Flyten har også guard mot partial reused audio og bedre repair/missing-media-håndtering.
